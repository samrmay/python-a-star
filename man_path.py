...
saved_grid = False
sprite_icon = pygame.image.load("C:/Users/samue/Python Projects/pathfinding_practice/chameleon.png")
sprite_speed = 2
...
#Define sprite class with update sprite function
class sprite:
    def __init__(self, pos, img, surface):
        self.pos = pos
        self.img = img
        self.screen = surface
    def draw(self, new_pos):
        self.screen.blit(self.img, (new_pos[0], new_pos[1]))
...
if not saved_grid:
    ...
    my_grid = []
start_node = my_grid[start_ID[0]][start_ID[1]]
icon = sprite((start_node.pos[0], start_node.pos[1]), sprite_icon, screen_base)
        ...
        if event.type == pygame.KEYDOWN:
            ...
            while True:
                for node in path:
                    travelling = True
                    pos_icon = node.pos
                    next_node_index = (path.index(node) + 1)
                    while travelling:
                        pos_icon[0] += sprite_speed
                        pos_icon[1] += sprite_speed
                        screen_base.fill(GREY)
                        draw_grid(my_grid)
                        icon.draw((pos_icon[0], pos_icon[1]))
                        pygame.display.update()
                        time.sleep(.001)
                        if next_node_index == len(path):
                            travelling = False
                        elif forward and pos_icon[0] >= path[next_node_index].pos[0] and pos_icon[1] >= path[next_node_index].pos[1]:
                            travelling = False
                        elif not forward and pos_icon[0] <= path[next_node_index].pos[0] and pos_icon[1] <= path[next_node_index].pos[1]:
                            travelling = False
                        for event in pygame.event.get():
                            if event.type == pygame.KEYDOWN:
                                exit()
